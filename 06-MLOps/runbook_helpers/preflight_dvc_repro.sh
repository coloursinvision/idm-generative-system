...
#
# Verifies eight pre-conditions required before running `dvc repro` on the
# IDM Generative System V2 ML pipeline (Layer 6). Designed to be invoked
# from the repository root on the droplet host
# (/opt/idm/idm-generative-system).
#
...
#
# Usage:
#   bash 06-MLOps/runbook_helpers/preflight_dvc_repro.sh
#   bash 06-MLOps/runbook_helpers/preflight_dvc_repro.sh --help
#
...
readonly DEFAULT_MLFLOW_URI="http://localhost:5000"
...
usage() {
    cat <<EOF
${SCRIPT_NAME} — pre-flight checks for DVC repro baseline (S8 Item 1)

Usage:
  bash ${SCRIPT_NAME}            run all eight checks
  bash ${SCRIPT_NAME} --help     show this message
...
check_05_mlflow_health() {
    ...
    if [[ "${code}" == "200" ]]; then
        ...
        fail "05-mlflow-health" "${uri}/health -> ${code}"
    fi
}

check_06_disk_space() {
    local free_gb
    free_gb=$(df -BG --output=avail . 2>/dev/null | awk 'NR==2 {sub("G$","",$1); print $1}')
...
check_08_mlflow_tracking_uri() {
    if [[ -z "${MLFLOW_TRACKING_URI:-}" ]]; then
        fail "08-tracking-uri" "MLFLOW_TRACKING_URI not exported"
        return
    fi
...
main() {
    ...
    printf "%spreflight: DVC repro baseline (S8)%s\n" "${COL_BOLD}" "${COL_RESET}"
    printf "  cwd:    %s\n" "$(pwd)"
    printf "  user:   %s@%s\n" "$(whoami)" "$(hostname -s)"
