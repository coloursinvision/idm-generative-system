...
### Added
- **`fm_analog` generator** — warm analog voice: two detuned FM oscillators → soft tanh saturation → resonant TPT state-variable low-pass (cutoff on an attack/decay envelope) → amplitude envelope. New additive generator; existing generators unchanged (D-DSP-02). Wired into `/generate` and `batch_export`; the lo-fi effects chain is intentionally NOT its default path (raw voice is the deliverable).
- **`fm_blip` FM expansion** — four additive, keyword-only, backward-compatible timbral controls (`mod_index_end`, `attack_ms`, `ratio`, `feedback`); a bare `fm_blip()` is bit-identical to the prior signal (D-DSP-01).

### Changed
- **`/compose` `reasoning`** returned as a top-level response field (was nested) — review M2/M3.
- **`/tuning/extract`** response field `model_version` → `model` (review mi1).
- **Audio scheduler** in both sequencer hooks driven by `setTimeout`, not `requestAnimationFrame` — keeps note scheduling alive in background tabs; `play()` re-entrancy guard added (D-CRF13-14).
- **ML pipeline integrity (training-time only; serving unchanged):**
  - Spec-level train/test leakage removed — group split by `spec_id` / `GroupShuffleSplit` (D-PIPE-05).
  - HPO-on-test fixed — held-out three-way group split; Optuna scores on validation, refit on train+val, metrics reported once on the held-out test (D-PIPE-06).
  - `tuning_hz` reframed out of the regression targets — deterministic A4 reference (constant 440.0), not a learned quantity; `/tuning` echoes the reference rather than predicting it (D-PIPE-07). `TuningEstimator` v6 @ Staging.
- **CI** now gates pull requests targeting `develop` (was `main`-only) — INF-X.
