...
DATASET_SCHEMA: pa.DataFrameSchema = pa.DataFrameSchema(
    columns={
        ...
        "spec_id": pa.Column(
            ...
            nullable=False,
            description=(
                "Global per-TrackSpec group key (enumerate index). Shared by "
                "all 1 + n_perturbations rows of one spec; used for "
                "group-aware train/test splitting. Not a model feature."
            ),
        ),
...
#                                                  not user-supplied)
#                spec_id, is_perturbed, perturbation_idx  (Layer 5 metadata only)
#
